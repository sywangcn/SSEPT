            ...
            for rel in relations:
                if rel["cause"] != 0:
                    ...
                    for sen in sentences:
                        # event1和event2在同一句话内
                        if event1_start_index >= sen["start"] and event1_end_index <= sen["end"] and event2_start_index >= sen["start"] and event2_end_index <= sen["end"]:
                             count += 1
    return count
def amr_parsing(file_name):
    ...
    with open(file_name, "r") as f:
        ...
        datas = json.load(f)
        af = open("C:\wsy\PycharmProjects\PCLearning\data\process_timebank.json", 'w')
        new_datas = []
        for i, data in enumerate(datas):
            ...
            for rel in relations:
                ...
                # af.write(sen)
                # af.write("\n")
                # 重定位2个触发词的位置：timebank中event1_start_index索引是从0开始的，sentences是从0开始的，每个句子从0开始
                e1_start = event1_start_index - sentence_start
                e1_end = event1_end_index - sentence_start
                e1 = " ".join(tokens[event1_start_index:event1_end_index+1])
                e2_start = event2_start_index - sentence_start
                e2_end = event2_end_index - sentence_start
                e2 = " ".join(tokens[event2_start_index:event2_end_index+1])
...
def shortest_path(file_name):
    ...
    jf = open("C:\wsy\PycharmProjects\PCLearning\data\shortest_path_timebank.json", "w")
    jf_new_datas=[]
...
if __name__=="__main__":
    # file_name = "C:\wsy\PycharmProjects\PCLearning\data\\timebank.json"
    # print(static_causality(file_name))
    # amr_parsing(file_name)
    file_name2 = "C:\wsy\PycharmProjects\PCLearning\data\process_timebank.json"
