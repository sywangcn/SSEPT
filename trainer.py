...
class Trainer(object):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # wsy：torch.nn.DataParallel 是一个模型包装器，用于在多个 GPUs 上并行运行模型。isinstance 函数用于检查 self.model 是否是 torch.nn.DataParallel 类的实例
            # if self.save_model:
        ...
        with open(os.path.join(self.output_dir, "metrics{0}.pkl".format(self.n_split)), "wb") as f:
            pickle.dump(self.epoch_metrics[res], f)