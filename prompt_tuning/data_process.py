...
def make_prompted(
    ...
    prompted_tokens = \
        ...
        new_tokens[target_start_index:target_end_index + 1] + ["."]+["The occurrence of event"]+new_tokens[source_start_index:source_end_index + 1]+\
        ["took place", T7, mask_token, T8, "event"]+new_tokens[target_start_index:target_end_index + 1]+["."]
...
def baseline_preprocess_data(data: Dict[str, Any], tokenizer: Union[BertTokenizer, RobertaTokenizer]) -> List[
    ...
    #         temporality = 0
        ...
        res.append(BaselineInputfeature(cause, -cause, temporality, -temporality, prompt1, prompt2))
    return res
    ...
    def __call__(self, data: List[BaselineInputfeature]) -> Tuple[torch.Tensor, ...]:
        ...
        return \
            input_ids1, masks1, input_ids_for_new1, mask_pos1, labels1, labels3,\
                input_ids2, masks2, input_ids_for_new2, mask_pos2, labels2, labels4
