...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        batch_size = q.size(0)
        q = self.q_linear(q).view(batch_size, -1, self.num_heads, self.dim_per_head).transpose(1, 2)
        k = self.k_linear(k).view(batch_size, -1, self.num_heads, self.dim_per_head).transpose(1, 2)
        v = self.v_linear(v).view(batch_size, -1, self.num_heads, self.dim_per_head).transpose(1, 2)
        attention_weights = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.dim_per_head)
        if mask is not None:
            attention_weights = attention_weights + mask.unsqueeze(1).unsqueeze(2)  # 应用掩码
...
class RNNModel(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        # 初始化隐状态和细胞状态
        h0 = torch.zeros(self.num_layers * (2 if self.lstm.bidirectional else 1), x.batch_sizes[0], self.hidden_size).to(x.data.device)
        c0 = torch.zeros(self.num_layers * (2 if self.lstm.bidirectional else 1), x.batch_sizes[0], self.hidden_size).to(x.data.device)
...
class BaselineFinetuningModel(torch.nn.Module):
    def __init__(self, mlm_name_or_path: str, args):
        ...
        # a代表非因果相关，b代表因果相关
        self.a = nn.Parameter(torch.randn(768))
        self.b = nn.Parameter(torch.randn(768))
        self.Iscausality_mlp = torch.nn.Linear(768, 2)
        """
        # 定义多头注意力
        self.multihead_attention = MultiheadAttention(768, 8)
        self.layernorm = nn.LayerNorm(768)
        """
        self.m = nn.GELU()

        # 定义RNN
        self.rnn = RNNModel(input_size=self.args.rnn_input, hidden_size=self.args.rnn_hidden, num_layers=self.args.rnn_num_layers, bidirectional=True)
    ...
    def forward_one_sentence(
        ...
        Temporal_masked_features = Causal_lm_head_output[torch.arange(batch_size), Temporal_masked_positions.reshape([-1]), :]  # batch_size*768
        ...
        for batch_idx, word_indices in enumerate(tokenize_ins_index):
            ...
            Iscausality_list = []
            # 添加位置编码
            sen_len = len(word_indices)
            pe = torch.zeros(sen_len, 768, device='cuda:0')
            position = torch.arange(0, sen_len).unsqueeze(1)
            div_term = torch.exp(torch.arange(0, 768, 2) *
                                 -(math.log(10000.0) / 768))
            pe[:, 0::2] = torch.sin(position * div_term)
            pe[:, 1::2] = torch.cos(position * div_term)
            ...
            for i, word_index in enumerate(word_indices):
                ...
                # 判断
                if i==0 or i==len(word_indices)-1:
                    Iscausality = self.Iscausality_mlp(word_vector)
                    Iscausality_lists.append(Iscausality)
                    Label = torch.argmax(Iscausality)
                    if Label==0:
                        word_vector = word_vector + self.a
                    else:
                        word_vector = word_vector + self.b

                new_sequence.append(word_vector)
            embeddings = torch.stack(new_sequence) + torch.tensor(pe[:sen_len], requires_grad=False)
            new_sequences.append(embeddings)
            sequence_lengths.append(len(new_sequence))

        Iscausality_label = torch.stack(Iscausality_lists).to("cuda:0")
        ...
        """
        # 创建掩码
        mask = torch.zeros_like(padded_sequences[:, :, 0])
        for i, length in enumerate(sequence_lengths):
            mask[i, length:] = -100000  # 将补全部分的掩码值设置为-100000

        # 多头注意力机制
        causal_attention_output = self.multihead_attention(Causal_masked_features.unsqueeze(1), padded_sequences,
                                                      padded_sequences, mask).reshape(batch_size, -1)
        temporal_attention_output = self.multihead_attention(Temporal_masked_features.unsqueeze(1), padded_sequences,
                                                        padded_sequences, mask).reshape(batch_size, -1)
        
        # 残差连接和归一化
        Causal_masked_features = causal_attention_output + Causal_masked_features
        # causal_attention_output = self.layernorm(causal_attention_output)
        Temporal_masked_features = temporal_attention_output + Temporal_masked_features
        # temporal_attention_output = self.layernorm(temporal_attention_output)
        """
        # 条件归一化
...
def get_optimizer(model: BaselineFinetuningModel, lr: float):
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            {"params": model.a},
            {"params": model.b},
            {"params": model.Iscausality_mlp.parameters()},
...
def batch_cal_loss_func(labels: Tuple[torch.Tensor, ...], preds: Tuple[torch.Tensor, ...], trainer):
    ...
    weights2 = torch.tensor([0.2, 1.0], device="cuda:0")
    Total_Loss = F.cross_entropy(pred1, labels1, reduction="mean", weight=weights) + F.cross_entropy(pred2, labels2, reduction="mean", weight=weights) \
        + F.cross_entropy(pred3, labels3, reduction="mean", weight=weights)+ F.cross_entropy(pred4, labels4, reduction="mean", weight=weights) \
                + F.cross_entropy(pred5, labels5, reduction="mean", weight=weights2)+F.cross_entropy(pred6, labels5, reduction="mean", weight=weights2)
