...
def make_prompted(
    ...
    prompted_tokens = \
        ...
        new_tokens[target_start_index:target_end_index + 1] + ["."]+["The occurrence of"]+new_tokens[source_start_index:source_end_index + 1]+\
        ["is", T7, mask_token, T8, "the occurrence of "]+new_tokens[target_start_index:target_end_index + 1]+["."]
    return " ".join(prompted_tokens)

def baseline_preprocess_data(data: Dict[str, Any], tokenizer: Union[BertTokenizer, RobertaTokenizer], trainORtest, count) -> List[
    ...
    res.append(BaselineInputfeature(cause, -cause, temporality, -temporality, prompt1, prompt2, tokenize_1t02_ins_index, tokenize_2t01_ins_index))

    if trainORtest=="valid":
        with open("C:\wsy\PycharmProjects\PCLearning\\amr_model\prompt_tuning\saved\\20240223_ESL_WithoutAMR.txt", 'a') as f:
            f.write(str(count)+". "+prompt1+"\n")
    return res
...
class BaselineCollator:
    ...
    def __call__(self, data: List[BaselineInputfeature]) -> Tuple[torch.Tensor, ...]:
        ...
        batch_labels_3, batch_labels_4 = [], [] # 保存时序标签
        batch_labels_5 = []
        tokenize_1t02_ins_index = []
        tokenize_2t01_ins_index = []
        ...
        labels3 = torch.tensor(data=batch_labels_3, dtype=torch.long)
        ...
        labels4 = torch.tensor(data=batch_labels_4, dtype=torch.long)
        ...
        return input_ids1, masks1, input_ids_for_new1, mask_pos1, labels1, labels3, tokenize_1t02_ins_index,\
                input_ids2, masks2, input_ids_for_new2, mask_pos2, labels2, labels4, tokenize_2t01_ins_index, labels5
