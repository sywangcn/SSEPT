...
@作者    :王松阳
@版本    :1.0
@修改     ：删除时序模板（关系到data_process.py文件）和时序损失；删除时序优化器；删除条件归一化；
'''
...
class BaselineFinetuningModel(torch.nn.Module):
    ...
    def forward_one_sentence(
        ...
        # input_embedding = input_embedding + torch.tensor(pe[:, :input_embedding.size(1)], requires_grad=False)
        ...
        # 分类器
        Causal_prediction = self.Causal_MLP(Causal_masked_features)

        return Causal_prediction, Iscausality_label
    ...
    def forward(
        ...
        pred1, pred5 = self.forward_one_sentence(input_ids1, masks1, input_ids_for_new1, mask_positions1, \
                                                        tokenize_1t02_ins_index)
        pred2, pred6 = self.forward_one_sentence(input_ids2, masks2, input_ids_for_new2, mask_positions2, \
                                                        tokenize_2t01_ins_index)
        return pred1, pred2, pred5, pred6
...
def batch_forward_func(batch_data, trainer):
    input_ids1, masks1, input_ids_for_new1, mask_pos1, labels1, tokenize_1t02_ins_index, \
        input_ids2, masks2, input_ids_for_new2, mask_pos2, labels2, tokenize_2t01_ins_index, labels5 = batch_data
    ...
    return (labels1, labels2, labels5), prediction


def batch_cal_loss_func(labels: Tuple[torch.Tensor, ...], preds: Tuple[torch.Tensor, ...], trainer):
    labels1, labels2, labels5 = labels
    pred1, pred2, pred5, pred6 = preds
