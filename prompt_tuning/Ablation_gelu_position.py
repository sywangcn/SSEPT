...
class BaselineFinetuningModel(torch.nn.Module):
    ...
    def forward_one_sentence(
        ...
        Causal_masked_features=self.dense1(Causal_masked_features)
        Temporal_masked_features = self.dense2(Temporal_masked_features)

        Causal_masked_features = self.gelu1(Causal_masked_features)

        Temporal_masked_features = self.gelu2(Temporal_masked_features)
        """
                    information fusion
                """
